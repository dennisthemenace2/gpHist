...
void CppHistVarianceFine(double* result,int numRows,int numCols,int numRows2,int numCols2,double* X,double* pred,double *lambda,int nlambda, double *vectors,double sigma,double* orders){
  ...
  for(int s=1; s<=numRows2;++s ){ // each sample to predict
    ...
    //std::cout<<"( k_dist - sumOfProjectionLengths ):"<<( k_dist - sumOfProjectionLengths )<<std::endl;
    
    
    //std::cout<<"last term"<<( 1.0 / lambda[nlambda-1] ) * ( k_dist - sumOfProjectionLengths )<<std::endl;
    ...
    //  std::cerr << "k_dist: " << k_dist << " - sumOfProjectionLengths: " <<  sumOfProjectionLengths << " is smaller than zero -possible ghost eigenvalues!"<<k_dist - sumOfProjectionLengths << std::endl;
  ...
  //  if(vMatResult(s,1)<0){
  //     std::cout<<"vMatResult(s,1)"<<vMatResult(s,1)<<std::endl;
  //  }
  }// end for each sample
...
void Cpplanczos(RMat& Xmat, RMat& bmat,RMat& vMatOrders,int k,double*alphas,double*betas,double sigma){
  ...
  //  betas[i] =  sqrt(w.SquareSum());
   ...
   // std::cout<<"sqrt(h1.SquareSum())"<<sqrt(h1.SquareSum())<<std::endl;
