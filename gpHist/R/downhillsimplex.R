...
downhillsimplexCCC = function(fc,nParams,bp=NULL,lower=0,upper=1,it=1000,tol=1e-10){
  ...
  if(is.null(bp)){
    #bp = matrix(runif( (N+1)*N,0,5),nrow=N+1,ncol=N )
    bp = matrix(0,nrow=nParams+1,ncol=nParams )
    for(i in 1:nParams){
      bp[,i] = runif( (nParams+1),lower[i],upper[i])
    }
  }else{
    if(nrow(bp) != nParams+1){
      print('solution points wrong dimension! your input will be replaced')
      bp = matrix(runif( (nParams+1)*nParams,0,5),nrow=nParams+1,ncol=N )
    }
    if(ncol(bp)>nParams){
      print('solution points dosnt need function value.')
      bp = bp[,1:nParams]
  ...
  #void Cdownhillsimplex(SEXP func,SEXP sys,int* nParams,double* bp,double* lower,double* upper, double alpha= 1,double gamma=2,double beta=0.5,double sigma=0.5,unsigned int it=1000,double tol=1e-10){
